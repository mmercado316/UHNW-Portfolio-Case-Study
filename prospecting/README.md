...
2026-04-24 (final pass). Strict-cut Notes & Sources from 34 numbered + 3 Additional context (37 total) down to 9 numbered citations. Dropped every source not directly cited or whose practitioner / concept was not named in the documents. Kept: Altrata SF, Spokesman-Review/Joint Venture, Beyer (Wealth Management Unwrapped + Relationship Alpha brief), Oechsli #1 Reason, Jaffe & Grubman Cross Cultures (Individualist concept), Kochis Managing Concentrated Stock (named in Prospect 1 outreach play; QSBS used 13× in body), Willis Dark Side of Wealth (third-person wealth language used in Prospect 3), Kitces 5-Step Prospecting (funnel-stage naming used in opening). Frontmatter source-count metric updated from "34 sources" to "9 sources" in both `website/deliverable.mdx` and `drafts/project_card.md`. Drafts/notes_and_sources.md mirrored. working_memory_v2.md NOTES & SOURCES untouched (internal research record).

2026-04-24 (later still). Refactored the Methodology section (formerly "Strategic Memo: Relationship intelligence beats ocean-boiling") in `website/deliverable.mdx`. Cut the entire `### Why templated outreach persists despite poor conversion` subsection — the templated-outreach failure is established once in the opening, not re-established here. Absorbed the Five Ps definition (People, Philosophy, Process, Performance, Fees) and the relationship alpha thesis into the start of "What the Prospect File actually solves" so the concepts are properly introduced before being referenced. Reworked all three remaining subsections into a cause-and-effect register (lead with what the Prospect File does, not with what templates fail to do). Renamed "From lead generation to relationship intelligence" → "What the Prospect File takes in" and cut its opening framing line. Applied de-template phrasing fixes throughout ("impossible to mistake for a template" → "specifically theirs" / "right opening for this specific prospect"). Source-count metric in frontmatter set to "34 sources" for precision (matches numbered citations after demoting Morgan Stanley/Goldman/Rockefeller AI-positions entry to non-numbered Additional context footer). Earlier same day: De-AI'd the case study. Renamed title from "The Client Book: AI-Augmented Prospecting for UHNW Clients" to "The Client Book: Relationship Intelligence for UHNW Prospecting" and slug from `ai-augmented-uhnw-prospecting` to `client-book-uhnw-prospecting`. Cut the Strategic Memo's `AI as build substrate` section entirely (~85 words). Rewrote the opening-preview line from "a view on where AI augments this work and where it must not replace it" to "the discipline that distinguishes relationship intelligence from data harvesting." Demoted the Morgan Stanley / Goldman / Rockefeller AI-positions citation (formerly #6) from numbered body-cited to a non-numbered "Additional context (not body-cited)" footer in Notes & Sources; renumbered former citations 7–35 down to 6–34. Changes mirrored in `drafts/strategic_memo.md`, `drafts/opening.md`, `drafts/project_card.md`, and the FRAME DECISIONS section of `working_memory_v2.md`. Frontmatter summary, Provenance-discipline AI mention in the Architecture section, and Prospect 3 / Prospect 4 biographical AI references all preserved (AI supplements the process but does not drive it).
